...
export default function AsiatiskMenuContent() {
  return (
    <div className="mx-auto mt-10 max-w-2xl rounded-xl border border-[#707164]/50 bg-[#12110D] p-6 text-center">
      ...
      <ul className="space-y-6" role="list">
        {ASIATISK_MENU_ITEMS.map((item) => (
          <li key={item.name} className="border-b border-[#707164]/30 pb-5 last:border-0 last:pb-0">
            ...
            {item.allergens && (
              <p className="mt-1 text-sm text-[#E5E7E3]/85">Allergener: {item.allergens}</p>
            )}
          </li>
        ))}
      </ul>
      <div className="mt-6 rounded-lg border border-[#707164]/40 bg-[#12110D]/80 p-4">
        <p className="text-sm font-semibold uppercase tracking-wide text-[#EAC84E]">Viktig information</p>
